...
class SVMModelTrainer(object):
    ...
    def __fn_impl(self, _df: pd.DataFrame):
        ...
        df["date"] = df.apply(
            lambda row: datetime.utcfromtimestamp(row.date).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            axis=1,
        )
        ...
        # Test data set
        X_test = X[split:-1]
        X_test = scaler.fit_transform(X_test.reshape(-1, X_test.shape[-1])).reshape(
            X_test.shape
        )
        y_test = y[split:-1]
        ...
        model_filename_path = os.path.dirname(
            ...
            self.fileToSave.split("/")[1].split(".")[0].strip(),
        ...
        bt = Backtest(
            bt_df, SVMStrategy, cash=cash, commission=0.0002, exclusive_orders=True
        )
        stat = bt.run()
        print(stat)
