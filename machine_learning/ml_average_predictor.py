...
class MLAveragePredictor:
    ...
    def __fn_impl(self, df: pd.DataFrame):
        ...
        try:
            model = load_model(
                os.path.dirname(os.path.abspath(__file__)) + model_filename_path
            )
            ...
            model.save(os.path.dirname(os.path.abspath(__file__)) + model_filename_path)
        ...
        valid["predictions_SMA_50"] = valid.predictions_SMA_50.shift(future_len)
        ...
        def plotFn():
            ...
            plt.legend(
                ["Train close", "Train SMA", "Valid close", "Valid SMA", "Predictions"],
                loc="lower right",
            )
