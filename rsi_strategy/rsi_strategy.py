...
class RSIStrategy(object):
    ...
    def execute(self, df):
        ...
        df['rsi_execute_buy'] = np.where(df['rsi_long_signal'] == True, df.close + 3 * df.STDEV_30, np.NaN)
        df['rsi_execute_sell'] = np.where(df['rsi_short_signal'] == True, df.close - 3 * df.STDEV_30, np.NaN)

        cash = 5_000
        def buy_pred_fn(row): return row.rsi_execute_buy != np.NaN
        def sell_pred_fn(row): return row.rsi_execute_sell != np.NaN
