...
class Patterns:
    ...
    def __fn_impl(self, stock_df: pd.DataFrame):
        ...
        kr = KernelReg(
            pd.to_numeric(stock_df["close"]), stock_df.index, var_type="c", bw=[0.85]
        )
        ...
        (
            ...
            smoothed_local_min,
            fit_fn,
        ) = self.__non_param_kernel_regression(kr, stock_df)
        ...
        for i in smoothed_local_max:
            if (i > 1) and (i < len(stock_df["close"]) - 1):
                price_local_max_dt.append(
                    pd.to_numeric(stock_df["close"]).iloc[i - 2 : i + 2].idxmax()
                )
        ...
        for i in smoothed_local_min:
            if (i > 1) and (i < len(stock_df["close"]) - 1):
                price_local_min_dt.append(
                    pd.to_numeric(stock_df["close"]).iloc[i - 2 : i + 2].idxmin()
                )

        max_min = pd.concat(
            [
                pd.to_numeric(stock_df.loc[price_local_min_dt, "close"]),
                pd.to_numeric(stock_df.loc[price_local_max_dt, "close"]),
            ]
        ).sort_index()
            ...
            for name, pattern_periods in patterns.items():
                if name == "HS":
                    ...
                    for start, end in pattern_periods:
                        ...
                        plot_window(
                            pd.to_numeric(stock_df["close"])[s:e],
                            max_min.loc[s:e],
                            smooth_prices[s:e],
                            smooth_extrema.loc[s:e],
                            ax=axes[i],
                        )
                        i += 1
                    plt.show()
        ...
        def plotFn():
            ...
            pd.to_numeric(smooth_prices).plot(ax=axs[1], legend=True, color="lightgray")
            axs[1].scatter(smooth_extrema.index, smooth_extrema.values, color="red")
